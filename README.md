...
В текущем проекте есть каталог `/docs/results`. Эта папка нужна как раз для того, чтобы вы могли поделиться результатами вашей работы. Вы можете записать скринкаст, сделать скриншоты или целый README-файл, в котором будет описано, что вы успели сделать.

Это один из критериев, по которому будет оцениваться ваша работа!

> **TODO**
>
> Добавить инструкцию к отправке работу (ссылку на форму с отправкой)
